...
class DatabaseManager:
    ...
    def migrate_database(self) -> None:
        ...
        try:
            with self.engine.connect() as conn:
                # Legacy media_items franchise columns (kept for backward compat, no migration risk)
                ...
                # Legacy collection_categories franchise columns (kept for backward compat)
                ...
                # New: collection_categories source tracking
                self._add_column_if_missing(conn, "collection_categories", "source", "VARCHAR(50) DEFAULT 'config'")
                self._add_column_if_missing(conn, "collection_categories", "natural_language_query", "TEXT")

                # Legacy item_classifications franchise columns (kept for backward compat)
    ...
    def create_natural_language_collection(self, session: Session, name: str,
                                          query: str, description: Optional[str] = None) -> CollectionCategory:
        """Create a collection category from a natural language query."""
        category = CollectionCategory(
            name=name,
            description=description or f"AI-generated collection: {query}",
            prompt=query,
        ...
        session.add(category)
        session.flush()
        logger.info(f"Created natural language collection: {name}")
        return category

    def get_natural_language_collections(self, session: Session) -> List[CollectionCategory]:
        """Get all collections created via natural language queries."""
        return session.query(CollectionCategory).filter(
            CollectionCategory.source == "natural_language"
        ).order_by(desc(CollectionCategory.created_at)).all()
    ...
    def get_database_stats(self, session: Session) -> Dict[str, int]:
        ...
        # Count natural language collections
        try:
            stats["nl_collections"] = session.query(CollectionCategory).filter(
                CollectionCategory.source == "natural_language"
            ).count()
        except Exception:
            stats["nl_collections"] = 0
